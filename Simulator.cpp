...
void checkSameNumInstErrorAlgoVsSimulation(int algoGetInsError, int simulationInstError, list<SimulatorError>& errorList){
    for(int i=10;i<19;i++){
...
bool Simulator::initAlgoWithTravelParam(Travel &travel, shared_ptr<AbstractAlgorithm> pAlgo, list<SimulatorError>& errorList){
...
void Simulator::runOneTravel(Travel &travel, shared_ptr<AbstractAlgorithm> pAlgo, const string &travelAlgoDirectory,
    ...
    bool dramaticProblem =initAlgoWithTravelParam(travel, pAlgo, errorList);
    ...
    while (!travel.didTravelEnd() && !dramaticProblem) {
        ...
        checkSameNumInstErrorAlgoVsSimulation(algoGetInsError,simulationInstError, errorList);
        FileHandler::simulatorErrorsToFile(errorList, travelErrorPath, travel.getTravelName(),
    ...
    errorList = {};
    checkIfShipEmpty(travel.getShipMap(), errorList, numberLoads, numberUnloads);
    FileHandler::simulatorErrorsToFile(errorList, travelErrorPath, travel.getTravelName());
}
